...
const readPaciente = async (id) => {
  ...
  if (!paciente) throw new ErrException(404, "error! Paciente not found");
  return paciente;
};
...
const createPaciente = async (pacienteBody) => {
  ...
  if (existPaciente) throw new ErrException(409, "Error! That email already exists");
  const result = await Paciente.create(pacienteBody);
  if (!result) throw new ErrException(500, "Couldn't create paciente");
  return result;
};
...
const deletePaciente = async (id) => {
  ...
  if (!result) throw new ErrException(500, "Couldn't delete paciente");
  return result;
};
...
const updatePaciente = async (id, pacienteBody) => {
  ...
  if (!result) throw new ErrException(500, "Couldn't update paciente");
